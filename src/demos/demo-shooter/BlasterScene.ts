...
export default class BlasterScene extends THREE.Scene {
    ...
    async initialize() {
        ...
        //cargar un MTL compartido para los targets (MTL=Material Template Library)
        const targetMtl = await this.mtlLoader.loadAsync('/assets/demo-shooter/targetA.mtl')
        targetMtl.preload()

        this.bulletMtl = await this.mtlLoader.loadAsync('/assets/demo-shooter/targetA.mtl')
        targetMtl.preload()
    ...
    private async createTarget(mtl: MTLLoader.MaterialCreator) {
        ...
        const modelRoot = await this.objLoader.loadAsync('/assets/demo-shooter/targetA.obj')
    ...
    private async createBlaster() {
        const mtl = await this.mtlLoader.loadAsync('/assets/demo-shooter/blasterG.mtl')

        this.objLoader.setMaterials(mtl)

        const modelRoot = await this.objLoader.loadAsync('/assets/demo-shooter/blasterG.obj')
    ...
    private async createBullet() {
        ...
        const bulletModel = await this.objLoader.loadAsync('/assets/demo-shooter/foamBulletB.obj')

        this.camera.getWorldDirection(this.directionVector)

        const aabb = new THREE.Box3().setFromObject(this.blaster) //Axis Aligned Bounding Box
